...
def load_data():
    ...
    retorno = pd.read_csv('./raw_data_pegi_pei_growth.csv')
...
with row[1].expander("Oportunidades de mercado"):
    ...
    header[1].markdown('<div style="text-align: right;">Valor importado: '+str(float(st.session_state['peso_importacao']))+'</div>',unsafe_allow_html=True)
...
df['valor_indice'] = peso_capacidade_atuais*(peso_valor_exportado*df['export_value_standarized'] + \
    ...
    peso_oportunidaes*(peso_valor_importado*df['import_value_standarized'] + peso_valor_importado_total*df['import_value_total_standarized']\
                   + peso_dcr*df['rcd_standarized'] +peso_crescimento*df['growth_standarized'] ) + \
...
df_plot = df[['valor_indice','hs_product_code','hs_product_name_short_en','export_value','rca','growth','density','import_value','import_value_total','rcd','pci','cog','pgi','pei']]
